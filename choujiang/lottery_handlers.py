    ...
    try:
        ...
        try:
            ...
            try:
                ...
                # 群组已迁移到超级群组，使用新的群组ID重试
                new_group_id = e.new_chat_id
                logger.info(f"[抽奖发布] 群组已迁移到超级群组，新ID: {new_group_id}，正在更新数据库并重试")
                
                # 更新数据库中的群组ID
                @sync_to_async
                def update_group_id(old_id, new_id):
                    from jifen.models import Group, User
                    from choujiang.models import Lottery
                    from django.db import transaction
                    with transaction.atomic():
                        # 检查新ID是否已存在
                        existing_group = Group.objects.filter(group_id=new_id).first()
                        old_group = Group.objects.get(group_id=old_id)
                        
                        if existing_group:
                            # 获取旧群组的所有用户
                            old_users = old_group.user_set.all()
                            for user in old_users:
                                # 检查用户是否已在新群组中
                                if User.objects.filter(telegram_id=user.telegram_id, group=existing_group).exists():
                                    # 如果用户已在新群组，删除旧群组中的用户记录
                                    user.delete()
                                else:
                                    # 如果用户不在新群组，更新用户的群组
                                    user.group = existing_group
                                    user.save()
                            
                            # 更新抽奖记录到新群组
                            for lottery in old_group.lottery_set.all():
                                # 检查是否存在相同ID的抽奖
                                existing_lottery = Lottery.objects.filter(id=lottery.id, group=existing_group).first()
                                if not existing_lottery:
                                    lottery.group = existing_group
                                    lottery.save()
                            ...
                            # 如果新ID不存在，直接更新旧群组的ID
                            old_group.group_id = new_id
                            old_group.save()
                            return old_group
                
                group = await update_group_id(group_id, new_group_id)
